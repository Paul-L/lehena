...
export async function retrieveCart(cartId?: string, fields?: string) {
  ...
  fields ??=
    "*items, *region, *items.product, *items.product.shipping_profile, *items.variant, *items.thumbnail, *items.metadata, +items.total, *promotions, +shipping_methods.name"
...
export async function applyGiftCard(_code: string) {
  // TODO(cart): implement gift card apply — awaits Medusa v2 gift card SDK exposure
}

export async function removeDiscount(_code: string) {
  // TODO(cart): implement discount removal
}
...
export async function removeGiftCard(
  ...
  // TODO(cart): implement gift card removal
}
...
export async function setAddresses(currentState: unknown, formData: FormData) {
  try {
    ...
    // FormData.get() renvoie FormDataEntryValue | null. Les adresses
    // Medusa attendent string | null | undefined. On normalise via un helper.
